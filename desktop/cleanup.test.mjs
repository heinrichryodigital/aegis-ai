...
async function fixture(t) {
  const temporary = await fs.realpath(
    await fs.mkdtemp(path.join(os.tmpdir(), "aegis-cleanup-test-")),
  );
...
test("preview includes only old, regular, singly linked eligible files", async (t) => {
  ...
  const cleanup = createCleanup({
    trashItem: async () => assert.fail("Preview must never mutate"),
  });
  ...
  assert.equal(
    results[0].size,
    Buffer.byteLength("Old expendable fixture log"),
  );
  assert.ok(
    Date.parse(results[0].modified) < Date.now() - CLEANUP_LIMITS.minAgeMs,
  );
...
test("only preview IDs invoke the injected Trash function, once", async (t) => {
  ...
  const result = await cleanup.trash({
    ids: [entry.id, entry.id, file, "unknown-id"],
  });
  ...
  assert.equal(
    ...
    "Old expendable fixture log",
    "No deletion fallback is permitted",
  );
});
...
test("modified files and a replaced inode are skipped after a preview", async (t) => {
  ...
  const cleanup = createCleanup({
    trashItem: async () => assert.fail("Changed files must never be trashed"),
  });
...
test("hardlinks introduced after preview prevent trashing", async (t) => {
  ...
  const cleanup = createCleanup({
    trashItem: async () => assert.fail("Hardlinked files are ineligible"),
  });
  const [entry] = await cleanup.preview({ root });
...
test("symlink parent replacement cannot escape the reviewed root", async (t) => {
  ...
  const cleanup = createCleanup({
    trashItem: async () => assert.fail("Parent links must never be followed"),
  });
  const [entry] = await cleanup.preview({ root });
  ...
  assert.equal(result.skipped, 1);
  assert.equal(
    await fs.readFile(path.join(outside, "old.log"), "utf8"),
...
test("rejects root, whole home, app code, linked roots and traversal", async (t) => {
  ...
  const cleanup = createCleanup({
    trashItem: async () => assert.fail("Root validation must not mutate"),
  });
...
test("rejects malformed cleanup requests before calling Trash", async () => {
  ...
  const cleanup = createCleanup({
    trashItem: async () => assert.fail("Invalid input cannot trigger Trash"),
  });
  for (const ids of [
    undefined,
    [],
    ...
    [123],
    ["x".repeat(101)],
    Array(CLEANUP_LIMITS.maxCandidates + 1).fill("id"),
  ])
    await assert.rejects(cleanup.trash({ ids }), TypeError);
});
