...
function harness({ dataDir = '/unused/aegis-test', provider, uuid, fileSystem = fs } = {}) {
  const events = new Map();
  const confirmations = [];
  const calls = { provider: 0, scannerClose: 0, watcherClose: 0, quit: 0 };
  const electron = {
    app: { whenReady: () => new Promise(() => {}), on: (name, callback) => events.set(name, callback), getPath: () => dataDir, quit: () => { calls.quit++; events.get('before-quit')?.({ preventDefault() {} }); } },
    ipcMain: {}, session: {}, shell: {},
    dialog: { showMessageBox: () => { const response = deferred(); confirmations.push(response); return response.promise; } },
  };
  const context = vm.createContext({
    require: id => id === 'electron' ? electron : id === 'node:fs/promises' ? fileSystem : id === 'node:crypto' && uuid ? { randomUUID: uuid } : realRequire(id),
    __dirname: desktopDirectory, URL, Promise, setTimeout, clearTimeout, setInterval, clearInterval,
  });
  ...
  `, context, { filename: 'desktop/main.cjs' });
  const controls = context.testControls;
  const frame = { url: controls.trustedURL };
  const webContents = { mainFrame: frame };
  const window = { webContents };
  controls.setServices({ window, providers: { analyzeReport: async request => { calls.provider++; return provider ? provider(request) : { text: 'Mock advice.' }; } } });
  return { controls, events, calls, confirmations, frame, webContents, event: { sender: webContents, senderFrame: frame } };
}
const request = { provider: 'codex', question: 'Review the summary.', report: {} };
...
test('IPC rejects foreign webContents, subframes, remote navigation and missing frame', async () => {
  ...
  await assert.rejects(controls.handleIPC({ ...event, sender: {} }, 'settings.get'), /Untrusted/);
  await assert.rejects(controls.handleIPC({ sender: webContents, senderFrame: { url: controls.trustedURL } }, 'settings.get'), /Untrusted/);
  await assert.rejects(controls.handleIPC({ sender: webContents }, 'settings.get'), /Untrusted/);
  ...
  assert.equal((await controls.handleIPC(event, 'settings.get')).watchDownloads, false);
...
test('analysis lock covers confirmation and execution; duplicate requests never prompt', async () => {
  ...
  const { controls, confirmations, calls } = harness({ provider: () => providerResult.promise });
...
test('failed provider execution releases the analysis lock', async () => {
  const { controls, confirmations, calls } = harness({ provider: () => { throw new Error('Mock provider failure'); } });
  const first = controls.handle('analyze', request);
  confirmations[0].resolve({ response: 1 });
...
test('invalid analysis payload is rejected before confirmation', async () => {
  ...
  for (const invalid of [{ ...request, report: undefined }, { ...request, report: [] }, { ...request, question: 'x'.repeat(2001) }, { ...request, provider: 'shell' }]) await assert.rejects(controls.handle('analyze', invalid), /Invalid analysis request/);
  assert.equal(confirmations.length, 0);
});

test('shutdown waits for scanner and watcher to close exactly once before quitting', async () => {
  const scannerDone = deferred(), watcherDone = deferred();
  const { controls, events, calls } = harness();
  controls.setServices({
    scanner: { close: () => { calls.scannerClose++; return scannerDone.promise; } },
    watcher: { close: () => { calls.watcherClose++; return watcherDone.promise; } },
  });
  controls.setSettings({ autoQuarantine: true, watchDownloads: true, watchNetwork: true });
  let prevented = 0;
...
test('settings temp creation is exclusive and cannot overwrite pre-existing data', async t => {
  ...
  if (process.platform !== 'win32') assert.equal((await fs.stat(path.join(dataDir, 'settings.json'))).mode & 0o777, 0o600);
});
...
test('settings writes reject symlink temp paths without modifying their targets', async t => {
  ...
  const temporary = path.join(dataDir, '.settings-test-id.tmp');
  try { await fs.symlink(target, temporary); }
  catch (error) { if (error.code === 'EPERM') { t.skip('Windows symlink permission unavailable.'); return; } throw error; }
  const { controls } = harness({ dataDir, uuid: () => 'test-id' });
  await assert.rejects(controls.persistSettings(), { code: 'EEXIST' });
