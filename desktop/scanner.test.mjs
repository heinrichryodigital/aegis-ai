...
async function fixture(t, onProgress) {
  const temporary = await fs.mkdtemp(
    path.join(os.tmpdir(), "aegis-scanner-test-"),
  );
...
test("ordinary files are not findings; EICAR is explicitly a harmless test", async (t) => {
  ...
  await fs.writeFile(
    ...
    "A regular user document.",
  );
  await fs.writeFile(
    path.join(root, "scanner-source.txt"),
    `const example = ${JSON.stringify(TEST_SIGNATURE)}`,
  );
...
test("quarantine and restart-safe restore preserve content and write durable audit", async (t) => {
  ...
  if (process.platform !== "win32")
    assert.equal(
      (await fs.stat(path.join(dataDir, "quarantine", record.id, "payload")))
        .mode & 0o777,
      0o600,
    );
  ...
  try {
    ...
    if (process.platform !== "win32")
      assert.equal((await fs.stat(original)).mode & 0o111, 0);
  } finally {
    await restarted.close();
  }
  const audit = (
    await fs.readFile(path.join(dataDir, "quarantine-audit.jsonl"), "utf8")
  )
    .trim()
    .split("\n")
    .map(JSON.parse);
  assert.deepEqual(
    ...
    ["quarantine-pending", "quarantined", "restore-pending", "restored"],
  );
});
...
test("mutated detections are refused without moving the changed file", async (t) => {
  ...
  assert.equal(
    ...
    "This file changed after its scan.",
  );
...
test("file and directory symlinks are not followed and symlink roots are rejected", async (t) => {
  ...
  try {
    await fs.symlink(
      path.join(outside, "test.com"),
      path.join(root, "linked.test"),
      "file",
    );
    await fs.symlink(
      outside,
    ...
    if (error.code === "EPERM") {
      t.skip("Windows symlink permission not available.");
      return;
    }
    throw error;
  }
  ...
  assert.equal(
    await fs.readFile(path.join(outside, "test.com"), "utf8"),
    TEST_SIGNATURE,
  );
  const linkedRootResult = await scanner.scan({
    root: path.join(root, "linked-directory"),
  });
...
test("replacing a detected file with a symlink cannot quarantine outside content", async (t) => {
  ...
  await assert.rejects(
    scanner.quarantine(result.findings[0].id),
    /regular files/i,
  );
...
test("hard-linked files are detected but not eligible for quarantine", async (t) => {
  ...
  assert.ok(
    result.findings.every(
      (finding) => !finding.quarantinable && !finding.quarantined,
    ),
  );
  await assert.rejects(
    scanner.quarantine(result.findings[0].id),
...
test("tampered quarantine payload cannot be restored", async (t) => {
  ...
  const [record] = await scanner.listQuarantine();
  await fs.writeFile(
    path.join(dataDir, "quarantine", record.id, "payload"),
    "Tampered storage",
  );
...
test("cancellation stops a running scan and concurrent scans are refused", async (t) => {
  let scanner;
  const setup = await fixture(t, (event) => {
    if (event.scanned === 1 && event.status === "scanning") scanner.cancel();
  });
